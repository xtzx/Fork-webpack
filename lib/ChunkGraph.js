...
/**
 * lib/ChunkGraph.js - Chunk 图（Chunk 和 Module 的关系图）⭐⭐⭐
 *
 * 【文件作用】
 * ChunkGraph 管理 Chunk 和 Module 之间的多对多关系
 *
 * 【核心概念】
 *
 * 1. **与 ModuleGraph 的区别**：
 *    - ModuleGraph: Module ↔ Module（模块依赖关系）
 *    - ChunkGraph: Chunk ↔ Module（包含关系）
 *
 * 2. **多对多关系**：
 *    - 一个 Chunk 包含多个 Module
 *    - 一个 Module 可以属于多个 Chunk（被共享）
 *
 * 【数据结构】
 *
 * ```
 ...
 *
 * ChunkGraphChunk {
 *   modules: Set<Module>          // 该 Chunk 包含的模块
 *   entryModules: Map<Module, Entrypoint>  // 入口模块
 *   runtimeModules: Set<RuntimeModule>     // 运行时模块
 * }
 *
 * ChunkGraphModule {
 *   chunks: Set<Chunk>            // 该模块属于哪些 Chunk
 *   entryInChunks: Set<Chunk>     // 作为入口的 Chunk
 *   runtimeInChunks: Set<Chunk>   // 作为运行时的 Chunk
 * }
 * ```
 *
 * 【使用示例】
 ...
 *
 * // 查询关系
 * const modules = chunkGraph.getChunkModules(chunk);  // Chunk 包含哪些模块
 * const chunks = chunkGraph.getModuleChunks(module);  // 模块属于哪些 Chunk
 *
 * // 计算大小
 ...
 *
 * 1. **代码分割优化**：
 *    查询模块属于哪些 Chunk，决定是否提取公共模块
 *
 * 2. **文件生成**：
 *    遍历 Chunk 的所有模块，拼接代码
 *
 * 3. **运行时生成**：
 *    确定每个 Chunk 需要哪些运行时模块
 *
 * 4. **大小计算**：
 *    计算 Chunk 的大小，用于性能预算
 *
 * 【创建时机】
 * 在 compilation.seal() 开始时创建：
 * ```javascript
 * const chunkGraph = new ChunkGraph(moduleGraph, hashFunction);
 * ```
 *
 * 【性能优化】
 * - 使用 Map 存储关系（O(1) 查询）
 * - 懒创建图节点（只在需要时创建）
 * - 使用 SortableSet（保持顺序，支持高效遍历）
 */
