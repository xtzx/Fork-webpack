 ...
 *
 * 【文件作用】
 * Module 是所有模块类型的基类，定义了模块的通用接口和行为
 *
 * 【核心职责】
 *
 * 1. **模块标识**：
 *    - identifier(): 模块的唯一标识符
 *    - readableIdentifier(): 人类可读的标识符
 *    - libIdent(): 库标识符
 *
 * 2. **依赖管理**（继承自 DependenciesBlock）：
 *    - dependencies: 依赖列表
 *    - blocks: 代码块列表（异步依赖）
 *    - addDependency(): 添加依赖
 *    - addBlock(): 添加代码块
 ...
 *   context: '/project/src',      // 上下文路径
 *   layer: null,                  // 图层
 *
 *   // ===== 依赖信息 =====
 *   dependencies: Dependency[],   // 依赖列表
 *   blocks: DependenciesBlock[],  // 代码块列表
 *
 *   // ===== 构建信息 =====
 *   buildInfo: {                  // 构建信息
 *     fileDependencies: Set,      // 文件依赖
 *     cacheable: true,            // 是否可缓存
 *     hash: 'a1b2c3'              // 内容哈希
 *   },
 *   buildMeta: {                  // 构建元数据
 *     exportsType: 'namespace',   // 导出类型
 *     sideEffectFree: false,      // 是否无副作用
 *     providedExports: ['foo']    // 提供的导出
 *   },
 *
 *   // ===== 其他 =====
 *   factoryMeta: {},              // 工厂元数据
 *   presentationalDependencies: [] // 展示性依赖
 * }
 * ```
 *
 * 【生命周期】
 *
 * ```
 * 创建: factory.create() → new Module()
 * 添加: compilation.addModule()
 * 构建: compilation.buildModule()
 *   ├─ module.needBuild() - 检查是否需要构建
 *   └─ module.build() - 执行构建
 * 代码生成: module.codeGeneration()
 * 输出: 包含在 chunk 中
 * ```
 *
 * 【关键方法】
 *
 * - build(): 子类必须实现，构建模块
 * - codeGeneration(): 子类必须实现，生成代码
 * - identifier(): 子类必须实现，返回唯一标识
 * - size(): 子类可以重写，返回模块大小
 *
 * 【设计模式】
 *
 * 1. **模板方法模式**：
 *    - 基类定义骨架（Module）
 *    - 子类实现细节（NormalModule.build）
 *
 * 2. **组合模式**：
 *    - Module 继承 DependenciesBlock
 *    - 可以包含子 DependenciesBlock
 *    - 形成树形结构
 *
 * 【文档中不存在的知识点】⭐⭐
 *
 ...
 *    - 运行时模块是代码生成的
 *
 * 2. **buildInfo vs buildMeta 的区别**：
 *    - buildInfo: 构建过程信息（缓存、依赖）
 *    - buildMeta: 构建结果元数据（导出、副作用）
 *
 * 3. **为什么继承 DependenciesBlock**：
 *    - 统一依赖管理
 *    - 支持嵌套结构
 *    - 简化遍历逻辑
 */
