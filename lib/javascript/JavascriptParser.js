...
/**
 * lib/javascript/JavascriptParser.js - JavaScript 解析器（AST 解析核心）⭐⭐⭐
 *
 * 【文件作用】
 * JavascriptParser 是 webpack 中最核心的解析器，负责：
 * 1. 将 JavaScript 源码解析为 AST（抽象语法树）
 * 2. 遍历 AST 节点识别依赖
 * 3. 创建各种 Dependency 对象
 * 4. 评估表达式的值（常量折叠）
 * 5. 处理作用域和变量
 *
 * 【核心职责】
 ...
 *    - 使用 acorn 解析源码为 AST
 *    - 支持 ES2020+ 语法
 *    - 支持 import assertions
 *
 * 2. **依赖识别**（最重要！）⭐⭐⭐：
 *    遍历 AST，识别各种依赖语句：
 *
 *    ES Module:
 *    - import foo from './a' → HarmonyImportDependency
 *    - export { foo } → HarmonyExportDependency
 *    - import('./a') → ImportDependency（动态导入）
 *
 *    CommonJS:
 *    - require('./a') → CommonJsRequireDependency
 *    - module.exports = {} → CommonJsExportsDependency
 *
 *    静态资源:
 *    - new URL('./a', import.meta.url) → URLDependency
 *    - new Worker('./w') → WorkerDependency
 *
 *    其他:
 *    - require.context() → ContextDependency
 *    - __webpack_require__() → 运行时依赖
 *
 * 3. **表达式评估**：
 *    - 常量表达式求值（1 + 2 → 3）
 *    - 条件判断（if (false) → 删除分支）
 *    - 变量追踪（const a = 1; use(a) → use(1)）
 *
 * 4. **作用域管理**：
 *    - 跟踪变量声明和使用
 *    - 识别自由变量（未声明的变量）
 ...
 * 源码 → parse() → AST
 *   ↓
 * walkStatements(ast.body)  // 遍历顶层语句
 *   ↓
 * 对每个节点类型调用对应的 walk 方法：
 *   ├─ ImportDeclaration → walkImportDeclaration()
 *   │   └─ 创建 HarmonyImportDependency
 *   ├─ ExportDeclaration → walkExportDeclaration()
 *   │   └─ 创建 HarmonyExportDependency
 *   ├─ CallExpression → walkCallExpression()
 *   │   ├─ require() → 创建 CommonJsRequireDependency
 *   │   └─ import() → 创建 ImportDependency
 *   ├─ VariableDeclaration → walkVariableDeclaration()
 *   └─ ... 其他节点类型
 *   ↓
 * 依赖收集到 module.dependencies
 * ```
 *
 * 【钩子系统】⭐⭐
 *
 * JavascriptParser 提供丰富的钩子供插件扩展：
 *
 * **表达式钩子**：
 * - expression: 遇到表达式时
 * - call: 遇到函数调用时
 ...
 * parser.hooks.import.tap('Plugin', (statement, source) => {
 *   // statement: AST 节点
 *   // source: './a.js'
 *   const dep = new HarmonyImportDependency(source);
 ...
 *
 * 2. **表达式评估缓存**：
 *    - 使用 BasicEvaluatedExpression 缓存求值结果
 *    - 避免重复计算
 *
 * 3. **作用域栈**：
 *    - 使用 StackedMap 高效管理作用域
 *    - O(1) 查找变量
 *
 * 【创建时机】
 * 由 NormalModuleFactory 根据模块类型选择解析器：
 * ```javascript
 * module.parser = compilation.createParser('javascript/auto');
 * ```
 *
 * 【调用时机】
 * 在 NormalModule.build() 中调用：
 * ```javascript
 * parser.parse(source, {
 *   current: module,
 *   module: module,
 ...
 *
 * 2. **如何处理动态导入**：
 *    - import() 是 ImportExpression 节点
 *    - 创建 ImportDependency（async: true）
 *    - 在 Seal 阶段创建新的 Chunk
 *
 * 3. **如何实现 Tree Shaking**：
 *    - 记录 import { foo } 使用了 'foo'
 *    - 记录 export const bar 提供了 'bar'
 *    - 对比找出未使用的导出
 *
 * 4. **常量折叠优化**：
 *    - if (false) { ... } → 整个分支删除
 *    - const a = 1; use(a) → use(1)
 *    - process.env.NODE_ENV === 'production' → true/false
 *
 * 5. **作用域提升**：
 *    - 追踪变量的声明和使用
 *    - 判断是否可以合并模块（Scope Hoisting）
 */

"use strict";

const { Parser: AcornParser } = require("acorn");  // acorn 解析器
const { importAssertions } = require("acorn-import-assertions");  // import 断言支持
const { SyncBailHook, HookMap } = require("tapable");  // 钩子系统
const vm = require("vm");  // 用于表达式求值
const Parser = require("../Parser");  // 解析器基类
const StackedMap = require("../util/StackedMap");  // 作用域栈
const binarySearchBounds = require("../util/binarySearchBounds");  // 二分查找
const memoize = require("../util/memoize");  // 记忆化
const BasicEvaluatedExpression = require("./BasicEvaluatedExpression");  // 表达式求值结果
...
class JavascriptParser extends Parser {
	 ...
	 *
	 * 【作用】
	 * 解析 JavaScript 源码，识别依赖，收集到 module.dependencies
	 *
	 * 【完整流程】
	 * ```
	 * parse(source, state)
	 *   ├─ 步骤1: 解析为 AST
	 *   │   └─ acorn.parse(source) → AST
	 *   │
	 *   ├─ 步骤2: 初始化作用域
	 *   │   └─ 创建顶层作用域（StackedMap）
	 *   │
	 ...
	 *   │
	 *   ├─ 步骤4: 三轮遍历 AST ⭐⭐⭐
	 *   │   ├─ detectMode() - 检测模块模式（严格模式等）
	 *   │   ├─ preWalkStatements() - 预遍历（收集声明）
	 *   │   ├─ blockPreWalkStatements() - 块预遍历
	 *   │   └─ walkStatements() - 主遍历（识别依赖）⭐⭐⭐
	 *   │
	 *   ├─ 步骤5: 触发 finish 钩子
	 *   │   └─ 插件可以后处理
	 *   │
	 ...
	 * 【三轮遍历的作用】⭐⭐
	 *
	 * **第 1 轮 - preWalkStatements（预遍历）**：
	 * 目的：收集所有声明，建立作用域
	 * ```javascript
	 * // 收集变量声明
	 * const foo = 1;  → 记录 foo 的声明
	 * function bar() {} → 记录 bar 的声明
	 * import x from './a' → 记录 x 的声明
	 * ```
	 *
	 * **第 2 轮 - blockPreWalkStatements（块预遍历）**：
	 * 目的：处理块级作用域的声明
	 * ```javascript
	 * {
	 *   const foo = 1;  // 块级声明
	 ...
	 * ```
	 *
	 * **第 3 轮 - walkStatements（主遍历）**：
	 * 目的：遍历所有语句，识别依赖
	 * ```javascript
	 * import foo from './a'  → HarmonyImportDependency
	 * require('./b')         → CommonJsRequireDependency
	 * import('./c')          → ImportDependency
	 * ```
	 *
	 * 【为什么需要三轮遍历】⭐
	 *
	 * 问题：变量可能先使用后声明（提升）
	 * ```javascript
	 * console.log(foo);  // 第 1 行使用
	 * var foo = 1;       // 第 2 行声明
	 * ```
	 *
	 * 解决：
	 * - 第 1 轮：收集所有声明
	 * - 第 3 轮：处理使用（此时已知道所有声明）
	 *
	 * 【ParserState（解析状态）】
	 * ```javascript
	 * state = {
	 ...
	 *   module: module,            // 模块引用
	 *   source: source,            // 源码
	 *   compilation: compilation   // 编译实例
	 * }
	 * ```
	 *
	 * 【性能考虑】
	 * - AST 解析占构建时间的 20-30%
	 * - 三轮遍历增加了时间，但提供了完整的语义分析
	 * - 缓存 AST 可以避免重复解析
	 *
	 * @param {string | Buffer | PreparsedAst} source - 源码（字符串、Buffer 或预解析的 AST）
	 * @param {ParserState} state - 解析器状态
	 * @returns {ParserState} 解析器状态
	 */
	parse(source, state) {
		let ast;         // AST 对象
		let comments;    // 注释列表
		const semicolons = new Set();  // 自动插入的分号位置
		...
		if (typeof source === "object") {
			 ...
			 *
			 * 【选项】
			 * - sourceType: 模块类型（'module'、'script'、'auto'）
			 * - onComment: 收集注释（用于提取魔法注释）
			 * - onInsertedSemicolon: 记录自动插入的分号
			 */
			ast = JavascriptParser._parse(source, {
				sourceType: this.sourceType,
				onComment: comments,  // 收集注释
				onInsertedSemicolon: pos => semicolons.add(pos)  // ASI 规则
			});
		}

		// ===== 步骤3: 保存旧状态（用于恢复）=====
		// 解析器可能被重入调用，需要保存和恢复状态
		 ...
		 *
		 * 【作用域属性】
		 * - topLevelScope: 是否顶层作用域
		 * - inTry: 是否在 try 块中
		 * - inShorthand: 是否在对象简写中
		 * - isStrict: 是否严格模式
		 * - isAsmJs: 是否 asm.js 模块
		 * - definitions: 变量定义映射（StackedMap）
		 */
		this.scope = {
			topLevelScope: true,        // 顶层作用域
			inTry: false,               // 不在 try 块中
			inShorthand: false,         // 不在对象简写中
			inTaggedTemplateTag: false, // 不在模板标签中
			isStrict: false,            // 非严格模式（detectMode 会更新）
			isAsmJs: false,             // 非 asm.js
		...
		this.statementPath = [];  // 语句路径（用于追踪位置）
		this.prevStatement = undefined;  // 上一个语句

		// ===== 步骤5: 触发 program 钩子 =====
		// 插件可以在这里预处理 AST 或返回结果跳过默认处理
		if (this.hooks.program.call(ast, comments) === undefined) {
			 ...
			 * 【作用】
			 * - 检测是否严格模式（'use strict'）
			 * - 检测是否 asm.js 模块（'use asm'）
			 * - 更新 scope.isStrict 和 scope.isAsmJs
			 */
			 ...
			 * ... 等等
			 *
			 * 所有依赖都添加到 state.module.dependencies
			 */
