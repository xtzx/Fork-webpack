 ...
 *
 * 【文件作用】
 * SplitChunksPlugin 是 webpack 最重要的优化插件之一，负责：
 * 1. 分析模块的共享情况
 * 2. 根据配置规则提取公共代码
 * 3. 创建新的 Chunk
 * 4. 优化 Chunk 的加载性能
 ...
 *      if (chunks.size > 1) {
 *        // 该模块被多个 chunk 共享，是候选模块
 *        candidates.add(module);
 *      }
 *    }
 *
 * 2. 分组阶段（按 cacheGroups 分组）：
 *    for (const module of candidates) {
 *      for (const cacheGroup of cacheGroups) {
 *        if (cacheGroup.test(module)) {
 *          groups[cacheGroup.key].add(module);
 *        }
 *      }
 *    }
 *
 ...
 *      // 应用 minSize
 *      if (calculateSize(group.modules) < minSize) {
 *        skip; // 太小，不值得提取
 *      }
 *
 *      // 应用 minChunks
 *      if (group.chunks.size < minChunks) {
 *        skip; // 共享不够，不提取
 *      }
 *
 *      // 应用 maxAsyncRequests
 *      if (asyncRequests > maxAsyncRequests) {
 *        只提取最大的几个;
 *      }
 *
 *      // 如果 enforce: true，忽略所有限制
 *    }
 *
 * 4. 创建阶段（创建新 Chunk）：
 *    for (const group of 通过过滤的分组) {
 ...
 * **minSize**: 最小体积（字节）
 *   - 默认: 20000 (20KB)
 *   - 作用: 过滤太小的模块，避免过多的 HTTP 请求
 *   - 实现: calculateSize(modules) >= minSize
 *
 * **minChunks**: 最小共享次数
 *   - 默认: 1
 *   - 作用: 模块至少被几个 chunk 使用才提取
 *   - 实现: moduleChunks.size >= minChunks
 *
 * **maxAsyncRequests**: 最大异步请求数
 *   - 默认: 30
 *   - 作用: 限制并行加载的文件数
 *   - 实现: 当超过限制时，只提取最大的几个模块
 *
 ...
 * **enforce**: 强制分割
 *   - 默认: false
 *   - 作用: 忽略 minSize、maxAsyncRequests 等限制
 *   - 使用场景: 强制提取 vendors
 *
 * **cacheGroups**: 缓存组配置
 *   ```javascript
 ...
 *       priority: 10,           // 优先级
 *       reuseExistingChunk: true  // 复用现有 chunk
 *     }
 *   }
 *   ```
 *
 * 【执行时机】
 * 在 compilation.hooks.optimizeChunks 钩子中执行（Seal 阶段）
 *
 * 【性能影响】
 * - 减少代码重复：节省 30-50% 体积
 * - 提升缓存效率：vendors chunk 很少变化
 * - 但会增加 HTTP 请求数（需要平衡）
 *
 * 【典型配置示例】
 *
...
module.exports = class SplitChunksPlugin {
	 ...
	 *
	 * 【注册时机】
	 * 在 compiler.hooks.thisCompilation 钩子注册
	 *
	 * 【执行时机】
	 * 在 compilation.hooks.optimizeChunks 钩子执行（Seal 阶段）
	 *
	 * 【执行阶段】
	 * STAGE_ADVANCED: 高级优化阶段
	 * 在基础 Chunk 创建之后，最终代码生成之前
	 *
	...
	apply(compiler) {
		...
		// ===== 注册到 thisCompilation 钩子 =====
		/**
		 * 在每次编译时注册优化逻辑
		 * 使用 thisCompilation 而不是 compilation，
		 * 确保只在主编译器执行，子编译器不执行
		 */
		compiler.hooks.thisCompilation.tap("SplitChunksPlugin", compilation => {
			...
			// ===== 注册到 optimizeChunks 钩子（核心逻辑）⭐⭐⭐ =====
			/**
			 * optimizeChunks 是代码分割执行的钩子
			 *
			 * 【执行阶段】
			 * STAGE_ADVANCED: 在基础优化之后执行
			 *
			 * 【执行时机】
			 * buildChunkGraph 完成后，代码生成前
			 * 此时所有模块已分配到 Chunk，可以分析共享情况
			 */
			compilation.hooks.optimizeChunks.tap(
				{
					name: "SplitChunksPlugin",
					stage: STAGE_ADVANCED  // 高级优化阶段
				},
				chunks => {
					// 防止重复优化（optimizeChunks 可能被多次调用）
